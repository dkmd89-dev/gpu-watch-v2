"""Preisstatistik-Aufbereitung fuer den Scan-Lauf (roadmap.md Phase 3,
Schritt 3.2).

Aus app.py extrahiert -- reine Berechnungs-/Reduktions-Helper auf Basis
der bereits vorhandenen Preishistorie (price_history.py/price_stats.py).
Kein eigener Zustand, keine Abhaengigkeit auf Scan-/Matching-Zustand.
Identisches Verhalten wie vorher: app.py importiert diese vier Funktionen
weiterhin unter denselben Namen, sodass bestehender Code (inkl. Tests,
die z.B. `app_mod._resale_prices_from_stats(...)` aufrufen) unveraendert
funktioniert.
"""
...
# Bewusst derselbe Logger-Name wie in app.py ("gpu-watch"), siehe
# persistence/json_store.py fuer die Begruendung (Log-Capture in Tests
# bleibt unabhaengig vom aufrufenden Modul funktionsfaehig).
